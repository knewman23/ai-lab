...
- [ ] **Step 1: Failing tests** (spec §3). `graphs.test.ts`: `GRAPH_KEYS` order neuron, product-sum, shared-node; `GRAPHS.neuron` has 10 nodes, output `o`, leaves `[x1, w1, x2, w2, b]` with the table's starts and ranges; node labels per spec; every `inputs` id exists; `shared-node` has non-leaf nodes ab (mul of a, b), e (add of ab, c), f (mul of e, c) and L (add of f, e), output L. `autograd.test.ts`: `topoOrder(neuron)` puts every input before its consumer, ends with `o`, is deterministic; `forward(neuron, starts(neuron))` → x1w1 −6, x2w2 0, sum −6, n 0.8814 (4 d.p.), o 0.7071; `backward` → o 1, n 0.5, sum 0.5, b 0.5, x1w1 0.5, x2w2 0.5, x1 −1.5, w1 1.0, x2 0.5, w2 0; product-sum d 4, grads a −3, b 2, c 1; shared-node ab −6, e 4, f 40, L 44 and full grads L 1, f 1, e 11, ab 11, a −33, b 22, c 15; `gradsAfter(shared-node, values, 1)` = { L: 1, f: 1, e: 1 } exactly, after 2 backward steps e 11 and c 4 (no ab/a/b keys yet), after 3 ab 11 and c 15, after 4 the full set; `localGrad(neuron, "o", 0, values)` ≈ central difference of tanh at n (rel 1e-4); `localGrad` add → 1, mul → the other input's value; `passSteps(neuron)` has 10 entries, first five `forward` in topo order of non-leaves, last five `backward` in reverse, `passSteps(product-sum)` 4, `shared-node` 8 (backward order L, f, e, ab); `revealed(neuron, 0)` = { values: the five leaves, backwardSteps: 0 }; after step 5 all ten values, backwardSteps 0; after step 6 backwardSteps 1 and `gradsAfter(…, 1)` has keys {o, n}; after step 7 keys {o, n, sum, b}; after 10 all ten with the notebook values; `backward` equals `gradsAfter` at the full count.
...
**Files:** create `src/viz/shared/labels.ts`; modify `styles/panel.css`; test `tests/viz/shared/labels.test.ts` (jsdom).

- [ ] **Step 1: Failing test.** `createLabelLayer(host)` appends one `div.viz-labels` to `host`; `set("a", "x1", [0, 0, 0], "node")` creates a span with class `node` and text "x1"; an `op` label's transform uses `translate(-50%, -50%)`; `set` again with new text updates in place (same element); `remove("a")` deletes it; `update(camera, 200, 200)` with a `PerspectiveCamera` at (0, −10, 0) looking at the origin (Z-up) puts the origin's span at translate(100px, 100px) (parse the `transform` style) and a point behind the camera (0, −20, 0) gets `hidden = true` (NOTE: `Vector3.project` maps it to NDC (0, 0, 1.02), the same x/y as the origin; only projected z > 1 tells them apart, so `projectToPixels` returns null when |z| > 1); a point projecting outside the canvas (NDC |x| or |y| > 1) is hidden; `update` calls `camera.updateMatrixWorld()` first (a bare test camera has a stale inverse, and in the app labels would lag the orbit by a frame); `clear()` removes every span; `dispose()` removes the div. Also a pure `projectToPixels(world, camera, w, h): [x, y] | null` used by `update`, tested on its own with two known points; `update` is a no-op when w or h is 0.
- [ ] **Step 2:** Run, confirm fail.
...
- [ ] **Step 1: Failing tests.** `stepText(d)` (pure): at step 0 "Leaves are given; press Step to run the forward pass."; forward step of `x1w1` → "forward: x1·w1 = 2 × −3 = −6" (the node's LABEL, the op applied to the input values, the result via `proseNum`; backward templates use node IDS before `.grad`); forward of `n` → "forward: n = −6 + 6.881 = 0.8814"; forward of `o` → "forward: o = tanh(0.8814) = 0.7071"; backward of `o` → "backward at o: o.grad = 1 → n.grad += 0.5 × 1"; backward of `n` (add) → "backward at n: n.grad = 0.5 → sum.grad += 1 × 0.5, b.grad += 1 × 0.5"; done → "Pass complete."; `edgeLabel(graph, node, inputIndex, values)` → "× −3" etc. Panel: graph select (three titles, `onGraph`); Step / Play–Pause / Reset pass buttons call `onStep`, `onPlay(on)`, `onResetPass`, and the Play button's label reads "Pause" when `state.playing`; leaf sliders rebuilt on graph change (5 for neuron, 3 for product-sum), each calls `onLeaf(id, v)` and reflects `state.leaves` on render without firing; Reset / Reset view; toggles Value bars / Grad bars / Edge derivatives → `onShow`; readouts: output row and one row per leaf with value and grad ("—" until revealed); the pass line "Step k of N: …"; equations via `createEquation` re-render only on graph change.
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** per spec §7; `panel-leaves.ts` owns the slider section (`createLeafSliders(section, graph, onLeaf)` → `{ render(leaves), dispose }`), rebuilt by `panel.ts` when the graph key changes. Use `createSlider({ label, min, max, step, value, onChange, format? })` from `src/ui/slider.ts` (setting `.value` does not fire `onChange`, which the no-fire render test relies on). Step 0.01.
...
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** mirroring `src/viz/chain-rule/index.ts` (pre-authorised split: put the per-apply label placement in `src/viz/backprop/labels-sync.ts` exporting `syncLabels(labels, graph, layout, d, show)` so index.ts stays near the sibling's size): `buildScene` (kit, wall, edges, nodes, bars, labels), `home = frameWall()`, `apply(next)`: derived → if graph changed: `edges.set`, `nodes.set`… (edges/nodes/bars all take the graph and layout each apply; they rebuild only on graph change internally) → `edges.setActive(current?.node ?? null)` → `nodes.set(revealed.values)` → `bars.set(graph, positions, values, grads, revealed, show, cause)` where `cause` is `"edit"` when the apply came from `setLeaf` (drag or slider) and `"step"` otherwise (steps, graph switch, mount: bars ease; edits jump) → labels: node labels for every node (plus `op` symbols centred on op spheres), value labels at value-bar tips (revealed and show.values; text via `fmt`), grad labels at grad-bar tips (revealed and show.grads; prefixed `∂ ` per spec §5), edge labels for the current backward step's input edges when `show.edgeDerivs` (removed otherwise) → `panel.render` → `dirty = true`. `update(dt)` (dt is SECONDS, capped at 0.1 by `core/loop.ts`; `STEP_MS` and `EASE_MS` are milliseconds, so accumulate `dt * 1000`): `controls.update`; if `state.playing`, accumulate against `STEP_MS` and `apply(stepForward(state))` when reached; `bars.update(dt * 1000)` → dirty while easing; return true while `state.playing` so the loop never idles between steps; when dirty, the camera moved, or `resize` ran since the last frame, `labels.update(camera, w, h)` (w/h kept from `resize`, defaulting to `canvas.clientWidth/Height` before the first resize; CSS pixels) and render. The label div is appended to `host.canvasContainer` (never `replaceChildren`, the canvas is already there). The label div is created before `createUsageHint` so the hint stays on top. One `attachDrag`: `hitTargets: bars.hitTargets` (a stable array of 5 boxes allocated once; unused ones are invisible after a graph switch, so no re-attach is needed), a plane containing the bar's axis and facing the camera: `plane: { normal: (i) => new Vector3(camera.position.x − X_i, 0, camera.position.z − Z_i).normalize() (written into a reused module Vector3), getOffset: (i) => that normal · (X_i − 0.12, 0, Z_i) }` (`attachDrag` calls `normal(i)` before `getOffset(i)` at pointerdown; verify the order in drag.ts and if `getOffset` runs first, compute both inside one helper called from `normal`), `onDrag(i, p)` → `apply(setLeaf(state, leafIds[i], −p[1] / S_VALUE))` (setLeaf clamps). Hint lines from spec §8. `resize(w, h)` stores w/h, updates the camera aspect and marks labels dirty. Dispose order: theme listener → drag → hint → labels → bars, nodes, edges, wall → `disposeObject(kit.scene)` → `kit.dispose()` → panel.
