...
- [ ] **Step 1: Failing tests** (spec §3). `graphs.test.ts`: `GRAPH_KEYS` order neuron, product-sum, shared-node; `GRAPHS.neuron` has 10 nodes, output `o`, leaves `[x1, w1, x2, w2, b]` with the table's starts and ranges; node labels per spec; every `inputs` id exists; `shared-node` has a node with inputs `[e, e]`. `autograd.test.ts`: `topoOrder(neuron)` puts every input before its consumer, ends with `o`, is deterministic; `forward(neuron, starts(neuron))` → x1w1 −6, x2w2 0, sum −6, n 0.8814 (4 d.p.), o 0.7071; `backward` → o 1, n 0.5, sum 0.5, b 0.5, x1w1 0.5, x2w2 0.5, x1 −1.5, w1 1.0, x2 0.5, w2 0; product-sum d 4, grads a −3, b 2, c 1; shared-node e 4, L 16, e.grad 8, a.grad −24, b.grad 16, c.grad 8; `localGrad(neuron, "o", 0, values)` ≈ central difference of tanh at n (rel 1e-4); `localGrad` add → 1, mul → the other input's value, mul with equal inputs → that value; `passSteps(neuron)` has 10 entries, first five `forward` in topo order of non-leaves, last five `backward` in reverse, `passSteps(product-sum)` 4, `shared-node` 6; `revealed(neuron, 0)` values = the five leaves, grads empty; after step 5 all values, grads empty; after step 6 (backward of `o`) grads {o, n}; after step 7 (backward of `n`) grads {o, n, sum, b}; after 10 all ten.
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** `autograd.ts` (`Op`, `GraphNode`, `Graph`, `Values`, `PassStep`, `topoOrder` (DFS post-order from the output, visiting inputs in order, matching the notebook), `forward`, `localGrad`, `backward` (output grad 1; reverse topo; `grads[input] += localGrad · grads[node]`), `passSteps`, `revealed`, helper `starts(g): Values`) and `graphs.ts` (`GraphKey`, `GRAPHS`, `GRAPH_KEYS`, the three presets with titles "tanh neuron", "a·b + c", "L = e·e", hints from the spec). `revealed` semantics exactly as spec §3: a grad is revealed for the node whose backward step ran and for each of its inputs.
...
- [ ] **Step 1: Failing test.** `createLabelLayer(host)` appends one `div.viz-labels` to `host`; `set("a", "x1", [0, 0, 0], "node")` creates a span with class `node` and text "x1"; `set` again with new text updates in place (same element); `remove("a")` deletes it; `update(camera, 200, 200)` with a `PerspectiveCamera` at (0, −10, 0) looking at the origin (Z-up) puts the origin's span at translate(100px, 100px) (parse the `transform` style) and a point behind the camera (0, −20, 0) gets `hidden = true`; a point projecting outside the canvas is hidden; `clear()` removes every span; `dispose()` removes the div. Also a pure `projectToPixels(world, camera, w, h): [x, y] | null` used by `update`, tested on its own with two known points.
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** per spec §5 "Labels": the div is `position: absolute; inset: 0; overflow: hidden; pointer-events: none` (CSS), spans `position: absolute; left: 0; top: 0; transform: translate(-50%, -100%) translate(xpx, ypx); white-space: nowrap; font: 12px var(--mono)`; kinds `node` (`color: var(--ink)`), `value` (`--soft`), `grad` (`--accent`), `edge` (pill: `background: var(--bg); border: 1px solid var(--line); padding: 0 4px; border-radius: 3px`). Check `styles/tokens.css` for the actual token names (`--mono` font var may differ; use what exists). `update` reuses one `Vector3` and writes `style.transform` only when the rounded pixel position changed.
...
- [ ] **Step 1: Failing tests** (spec §4, §6). Layout: `layoutGraph(GRAPHS.neuron)` gives 5 distinct X columns evenly placed in [−5, 5] (X = −5 + (depth + 0.5)·2), leaf column Z order x1, w1, x2, w2, b top (5.2) to bottom (0.8), `o` at (4, 3); every edge goes to a strictly greater X; no two nodes share a position; `product-sum` has 3 columns. State: `initialState()` (neuron, leaves = starts, step 0, playing false, show all true); `setGraph` resets leaves/step/playing; `setLeaf` clamps to the leaf's range and keeps step; `stepForward` increments to the pass length then stops and clears `playing`; `resetPass`; `setPlaying`; `setShow`; `reset`; `derived` at initial state: values/grads equal the math helpers, `steps.length` 10, `current` null, `phase` "idle", `done` false; after 3 steps `phase` "forward", `current` is `steps[2]`; after 6 `phase` "backward"; after 10 `done` true and `phase` "done".
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** `layout.ts` (`depthOf` = longest path from a leaf; columns; rows ordered by mean input Z, leaves by declaration order; export `WALL_W = 10`, `WALL_H = 6`, `Z_RANGE = [0.8, 5.2]`) and `state.ts` (`BpState`, `ShowKey`, reducers, `Derived` with `graph, values, grads, steps, current, revealed, done, phase`). Export `STEP_MS = 700`.
...
- [ ] **Step 2:** Run, confirm fail.
- [ ] **Step 3: Implement** mirroring `src/viz/chain-rule/index.ts`: `buildScene` (kit, wall, edges, nodes, bars, labels), `home = frameWall()`, `apply(next)`: derived → if graph changed: `edges.set`, `nodes.set`… (edges/nodes/bars all take the graph and layout each apply; they rebuild only on graph change internally) → `edges.setActive(current?.node ?? null)` → `nodes.set(revealed.values)` → `bars.set(values, grads, revealed, show)` → labels: node labels for every node, value labels at value-bar tips (revealed and show.values), grad labels at grad-bar tips (revealed and show.grads), edge labels for the current backward step's input edges when `show.edgeDerivs` (removed otherwise) → `panel.render` → `dirty = true`. `update(dt)`: `controls.update`; if `state.playing`, accumulate dt and `apply(stepForward(state))` every `STEP_MS`; `bars.update(dt)` → dirty while easing; when dirty or the camera moved, `labels.update(camera, w, h)` (keep w/h from `resize`) and render. One `attachDrag`: `hitTargets: bars.hitTargets`, `plane: { normal: (1, 0, 0), getOffset: (i) => layout[leafIds[i]][0] }`, `onDrag(i, p)` → `apply(setLeaf(state, leafIds[i], −p[1] / S_VALUE))` (setLeaf clamps). Hint lines from spec §8. Dispose order: theme listener → drag → hint → labels → bars, nodes, edges, wall → `disposeObject(kit.scene)` → `kit.dispose()` → panel.
