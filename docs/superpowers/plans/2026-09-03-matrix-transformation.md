...
- [ ] **Step 1: Failing test** — extend the theme test: a stub `read` returning distinct hex for `--warn` and `--line-2`; `colors.warn.getHexString()` and `colors.line2.getHexString()` match; `refresh()` updates both in place.
- [ ] **Step 2: Implement** — add `warn: Color; line2: Color` to `ThemeColors` (keep the interface comment), the two token entries in `TOKENS` (the `satisfies` guard forces this) and the two fields in the implementation. Confirm both tokens are 6-digit hex in all three blocks of `styles/tokens.css` (they are: `--warn` #85670f / #e2b357, `--line-2` #cfcfcb / #2e323b).
- [ ] **Step 3: `pnpm check`; commit** — "Expose warn and line-2 tokens to the scene theme".
...
- [ ] **Step 1: Failing tests** — `createSlider({ label, min, max, step, value, onChange, format? })` → `{ el, get value, set value }`: native range with the given min/max/step; `format` default `(v) => fmt(v, 3)`; `input` dispatches `onChange` with the numeric value; programmatic `.value = 0.5` moves the input and output without `onChange`; output text uses `format`. `createSelect` options gain `disabled?: boolean`: a disabled option renders with the `disabled` attribute; programmatic `.value = "custom"` still selects it.
- [ ] **Step 2: Implement** — share the DOM-building helpers between `createLogSlider` and `createSlider` (extract a private `buildSlider` in `slider.ts`; both public functions stay). Additive `disabled` on `SelectOption`.
- [ ] **Step 3: `pnpm check`; commit** — "Add a linear slider and disabled select options".
...
- [ ] **Step 2: Failing tests for the generalised APIs** (spec §8):
  - `frameFor(domain: { x: readonly [number, number]; y: readonly [number, number] }, heightRange)`: existing bowl/rosenbrock assertions rewritten to pass `surface.domain`; new case: domain [−5, 5]², heightRange [0, 0] → target (0, 0, 0), position (10, −11.5, 8.5).
  - `attachDrag({ canvas, camera, controls, hitTargets, getPlaneZ(index), clamp?, enabled?, surfaceTarget?, onDrag(index, pos) })`: existing cases updated; new: two hit targets → `onDrag` reports index 1 when the second is grabbed; `enabled: () => false` → no drag starts and cursor stays ""; click-to-place calls `onDrag(-1, pos)`; when `surfaceTarget` is omitted a click does nothing; `getPlaneZ(index)` receives the grabbed index.
...
- [ ] **Step 3: Implement the generalisations** and update the gradient caller: one hit target, `getPlaneZ: () => s·f(pos)`, `clamp: (p) => clampToDomain(SURFACES[state.surface], p)`, `surfaceTarget: surfaceMesh.group`, `onDrag: (_i, p) => apply(drag(state, p))`; hint with `storageKey: "ai-lab.hint.gradient-descent"`, its heading and the three existing lines. `drag.ts` must no longer import anything from `core/math/surfaces`.
- [ ] **Step 4: `pnpm check`; verify the gradient scene in Chrome** (drag, click-to-place, hint) still works at http://localhost:5173/ai-lab/#/machine-learning/gradient-descent; commit "Generalise drag, framing and hint for reuse across visualizations".
...
- [ ] **Step 2: Implement**; `pnpm check`; commit "Add matrix transformation presets and pure state".
...
- [ ] **Step 2: Implement** with a `.matrix-input` grid (CSS in `styles/panel.css`, tokens only, mono font, two columns). Commit "Add the 2x2 matrix input block".
...
- [ ] **Step 1: Implement** `createPlane(theme, bound = 5)` → `{ group, setMatrix(mt: Mat2, detMt: number), setShow({ grid, ghost }), dispose }` with four layers per spec §3, each material `transparent: true, depthTest: false, depthWrite: false`, `renderOrder` 1 (reference grid, `--line`), 2 (transformed grid, `--soft`), 4 (ghost square outline, `--faint`), 5 (fill). Transformed grid: 14 lines → 84-float buffer, `clipSegment` per line against `bound`, compacted, draw range set. Fill: a 4-vertex `BufferGeometry` quad (two triangles) whose positions are rewritten per `setMatrix`; colour `--accent` / `--warn` by sign of `detMt`; when |detMt| < 1e-6 rewrite the quad as the 0.04-wide segment spanning the collinear mapped corners (spec §3), colour `--warn`; if all corners coincide set draw range 0. Recolour on theme "change". `dispose` removes the listener, disposes geometries/materials, `group.clear()`.
...
- [ ] **Step 1: Failing tests** — `createMtPanel(host, handlers, info)` with `handlers = { onPreset, onEntry, onT, onReset, onResetView, onShow }`: renders readouts for `initialState()` (det 1, trace 2, eigenvalues "1 (all directions)", area 1, "preserved"); reflection preset → det −1 and "reversed"; rotation → "complex pair"; shear → one eigenvalue "1"; projection → "collapsed"; changing the preset select dispatches `onPreset("shear")`; render with t 0.5 shows the note "Set Animate to 1 to drag the vectors" and hides it at t 1; render does not re-fire handlers.
- [ ] **Step 2: Implement** panel per spec §6 (sections: Setup [preset select, matrix input, animate slider], Run [Reset, Reset view], Show [Transformed grid, Eigenvectors, Ghost square], Readouts, note, explanation) and `explanation.ts` (three paragraphs with `createEquation`; the matrix rendered as `\begin{pmatrix}` with live `fmt(v, 3)` numbers; det sentence with the t < 1 clause; eigen sentence per kind incl. the projection wording). Commit "Add matrix transformation panel and explanation".
