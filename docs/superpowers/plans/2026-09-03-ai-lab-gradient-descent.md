...
- Test: `tests/ui/slider.test.ts`, `tests/ui/readout.test.ts` (jsdom)
...
- `step` that lands outside the domain → `status: "left-domain"`, `running: false`, `canStep: false`; `pos` is the finite out-of-domain point (the readout shows it alongside "left the domain"). Deterministic setup: `saddle` from its start, SGD, lr 0.1; step until the status changes and assert it happens within 100 steps.
- Divergence: a learning rate large enough that the first SGD step overflows to ±Infinity on `rosenbrock` (lr 1 exits the domain while still finite, which is the `"left-domain"` case, not divergence) → status `"diverged"`, `pos` has a non-finite component, no throw.
...
1. `kit = createSceneKit(host.renderer, host.theme, { reducedMotion: prefersReducedMotion() })` with `camera.up.set(0,0,1)`. Framing is a pure helper `frameFor(surface, heightRange): { position, target }`: target = (domain centre x, domain centre y, heightRange mid); camera = target + `halfExtent · (2, −2.3, 1.7)` where `halfExtent` is the larger domain half-width (so `himmelblau` and `rosenbrock` frame correctly). Store the pose for "Reset view"; refresh it on every re-frame (surface change). Mount ends by calling `apply(initialState())` with no previous key so the first surface sync, framing and pose storage happen there.
2. Build surface mesh, contour lines, marker, path line; add to `kit.scene`. `attachDrag({ canvas: host.renderer.domElement, camera, controls, marker, getSurface, onDrag })`.
...
- [ ] **Step 4: Verify**
Run (non-interactive; bare `gh run watch` prompts). Right after the push `gh run list` may be empty for a few seconds; if the id is empty, wait 10 s and retry:
```bash
