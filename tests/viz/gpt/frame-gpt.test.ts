...
import { FLOOR_Y, WALL_H } from "../../../src/viz/gpt/layout";
...
describe("frameGpt", () => {
  ...
  it("keeps the whole wall and floor in front of it, and is stable across calls", () => {
    const a = frameGpt();
    expect(a.position[1]).toBeLessThan(FLOOR_Y[0]);
    expect(frameGpt()).toEqual(a);
  });
});
